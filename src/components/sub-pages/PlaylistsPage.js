...
export const PlaylistsPage = () => {
    const [{ token, selectedId, playlists}, dispatch] = useStateProvider();

    useEffect(() => {
        const getPlaylistData = async () => {
            try {
                const response = await axios.get(
                    `https://api.spotify.com/v1/browse/categories/${selectedId}/playlists`,
                    {
                        headers: {
                        ...
                        if(err.message.includes("404")){
                            alert("Please try some other Category")
                            dispatch({ type: reducerCases.SET_SCREEN, screen: "CategoryPage" });
                        }
                    })
                const temp = response.data
                const playlists = temp.playlists.items
    ...
    return (
        <div className='flex flex-col w-full overflow-auto items-center'>
            <label className='text-3xl my-4'>PLAYLISTS</label>
            <Grid
                container
                columns={{ xs: 4, md: 12 }}
